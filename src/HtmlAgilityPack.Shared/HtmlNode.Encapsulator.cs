...
public partial class HtmlNode
{
    ...
    /// <exception cref="XPathException"><see cref="System.Xml.XPath.XPathException"/></exception>
    ...
    public T? GetEncapsulatedData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] T>()
    {
    ...
    /// <exception cref="Exception">Why it's thrown.</exception>
    /// <exception cref="InvalidNodeReturnTypeException"><see cref="InvalidNodeReturnTypeException"/></exception>
    public T? GetEncapsulatedData<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] T>(HtmlDocument htmlDocument)
    {
    ...
    /// <exception cref="Exception">Why it's thrown.</exception>
    /// <exception cref="InvalidNodeReturnTypeException"><see cref="InvalidNodeReturnTypeException"/></exception>
    public object? GetEncapsulatedData([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] Type targetType, HtmlDocument? htmlDocument = null)
    {
            ...
            else
            {
                ...
                foreach (PropertyInfo propertyInfo in validProperties)
                {
                    ...
                    if (propertyInfo.IsIEnumerable() == false) // Property is None-IEnumerable
                    {
                        ...
                        if (htmlNode is null) // If Encapsulator could not find Node.
                        {
                        ...
                        else // if htmlNode is not null (Encapsulator find the Node)
                        {
                            ...
                            {
                                HtmlDocument innerHtmlDocument = new();
...
#pragma warning restore IL2072 // Target parameter argument does not satisfy 'DynamicallyAccessedMembersAttribute' in call to target method. The return value of the source method does not have matching annotations.
                            ...
                            else
                            {
                                ...
                                if (xPathAttribute?.AttributeName is null) // It target value of HTMLTag
                                {
                    ...
                    else // Property is IEnumerable<T>
                    {
                        ...
                        if (propertyInfo.GetGenericTypes() is not IList<Type> T_Types || T_Types.Count == 0)
                        {
                        ...
                        else if (T_Types.Count == 1) // It is NOT something like Dictionary<Tkey , Tvalue>
                        {
                            ...
                            if (nodeCollection is null || nodeCollection.Count == 0)
                            {
                                if (propertyInfo.IsDefined(typeof(SkipNodeNotFoundAttribute), false) == true)
                                {
                                    // set default value.
...
#pragma warning restore IL2072 // Target parameter argument does not satisfy 'DynamicallyAccessedMembersAttribute' in call to target method. The return value of the source method does not have matching annotations.
                                ...
                                {
                                    if (xPathAttribute?.AttributeName is null) // It target value
                                    {
                                        try
                                        {
                                    ...
                                    else // It target attribute
                                    {
                                        ...
                                        foreach (HtmlNode? node in nodeCollection)
                                        {
                                            string? nodeAttributeValue = (node?.GetAttributeValue(xPathAttribute.AttributeName, null)) ?? throw new NodeAttributeNotFoundException("Can not find " + xPathAttribute.AttributeName + " Attribute in " + node?.Name + " related to " +
                                                propertyInfo.PropertyType.FullName + " " + propertyInfo.Name);
                                            object resultCastedToTargetPropertyType;
...
#pragma warning restore IL2070 // 'this' argument does not satisfy 'DynamicallyAccessedMembersAttribute' in call to target method. The parameter of method does not have matching annotations.
    ...
    {
        //return propertyInfo.PropertyType.GetInterface(typeof(IEnumerable<>).FullName) is not null;
...
#pragma warning restore IL2070 // 'this' argument does not satisfy 'DynamicallyAccessedMembersAttribute' in call to target method. The parameter of method does not have matching annotations.
    ...
    internal static T? GetNodeValueBasedOnXPathReturnType<T>(HtmlNode htmlNode, XPathAttribute? xPathAttribute)
    {
        ...
        ArgumentNullException.ThrowIfNull(xPathAttribute);
    ...
    internal static IList? GetNodesValuesBasedOnXPathReturnType(HtmlNodeCollection htmlNodeCollection, XPathAttribute? xPathAttribute, Type listGenericType)
    {
        ...
        {
            throw new ArgumentNullException(nameof(htmlNodeCollection));
        }
...
#pragma warning restore IL2070 // 'this' argument does not satisfy 'DynamicallyAccessedMembersAttribute' in call to target method. The parameter of method does not have matching annotations.
    ...
    internal static string? GetHtmlForEncapsulation(HtmlNode? node, ReturnType? returnType)
    {
        return returnType switch
        {
            ...
            _ => throw new InvalidNodeReturnTypeException(string.Format("Invalid ReturnType value {0}", returnType)),
        };
        ;
    }
}
