...
/// </summary>
    ...
    public static string? GetContentTypeForExtension(string extension, string? def)
    {
        ...
        if (!MimeTypeMap.Mappings.TryGetValue(extension, out string? contentType))
        {
            contentType = def;
        }
    ...
    public static string GetExtensionForContentType(string contentType, string def)
    {
        ...
        if (!MimeTypeMap.Mappings.TryGetValue(contentType, out string? ext))
        {
            ext = def;
        }
...
/// <param name="proxy"></param>
    ...
    {
        //Create my proxy
        WebProxy myProxy = new(proxyHost, proxyPort)
        {
...
#pragma warning restore SYSLIB0014 // Type or member is obsolete
        ...
        ICredentials creds)
    {
        ...
        using (var client = new HttpClient(handler))
        {
            ...
            bool isUnknown = response.Content.Headers.ContentType is null;
            ...
            if (s is not null)
            {
                ...
                else
                {
                    ...
                    else if (doc is not null && isUnknown)
                    {
                        try
                        {
                            if (respenc is null)
...
#if !(NETSTANDARD1_3 || NETSTANDARD1_6)
    private static bool IsCacheHtmlContent(string path)
    {
    ...
    {
        return contentType is not null && contentType.StartsWith("text/html", StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task<HtmlDocument> LoadFromWebAsync(Uri uri, Encoding encoding, NetworkCredential credentials, CancellationToken cancellationToken)
    {
        ...
        if(credentials is not null || CaptureRedirect)
        {
        ...
        if(e.Headers is not null && e.Headers.Location is not null)
        {
    ...
    public HtmlDocument LoadFromBrowser(string url, Func<object, bool> isBrowserScriptCompleted = null)
    {
        ...
        if (system_windows_forms is null)
        {
            // TRY to load id
            try
            {
